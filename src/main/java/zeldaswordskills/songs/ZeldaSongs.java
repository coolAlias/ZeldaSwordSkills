...
public final class ZeldaSongs
{
	...
	public static void register(AbstractZeldaSong song) {
		if (songNames.contains(song.getUnlocalizedName()) || songMap.containsKey(song.getUnlocalizedName())) {
			LogHelper.warning(String.format("Failed to register %s: Unlocalized name must be unique.", song.getUnlocalizedName()));
		} else if (!ZeldaSongs.areNotesUnique(song.getNotes())) {
			LogHelper.warning(String.format("Failed to register %s: Notes provided are not unique %s", song.getUnlocalizedName(), song.getNotes()));
		} else {
			songNames.add(song.getUnlocalizedName());
			songMap.put(song.getUnlocalizedName(), song);
			LogHelper.info(String.format("Registered %s as '%s' with notes %s", song.getDisplayName(), song.getUnlocalizedName(), song.getNotes()));
		}
	}
