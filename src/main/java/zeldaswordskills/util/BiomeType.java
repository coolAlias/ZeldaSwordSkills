...
public enum BiomeType {
	ARID("arid", "Desert", "Desert Hills"),
	BEACH("beach", "Beach"),
	COLD("cold", "Frozen River", "Ice Mountains", "Ice Plains"),
	FIERY("fiery", "Hell"),
	FOREST("forest", "Forest", "Forest Hills"),
	JUNGLE("jungle", "Jungle", "Jungle Hills"),
	MOUNTAIN("mountain", "Extreme Hills", "Extreme Hills Edge"),
	OCEAN("ocean", "Ocean", "Frozen Ocean"),
	PLAINS("plains", "Plains"),
	RIVER("river", "River", "Swampland"),
	SHROOM("shroom", "MushroomIsland", "MushroomIslandShore"),
	TAIGA("taiga", "Taiga", "Taiga Hills");
	...
	private static void addBiomes(BiomeType type, String[] biomeNames) {
		for (String biome : biomeNames) {
			...
			if (!isRealBiome(biome)) {
				LogHelper.warning(String.format("%s is not a recognized biome! This entry will be ignored for BiomeType %s", biome, type.toString()));
			} else if (biomeTypeList.containsKey(biome)) {
				LogHelper.warning(String.format("Error while adding %s for %s: biome already mapped to %s", biome, type.toString(), biomeTypeList.get(biome).toString()));
			} else {
