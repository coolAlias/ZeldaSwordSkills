...
{
	private static final MerchantRecipe standardBomb = new MerchantRecipe(new ItemStack(Item.emerald, 8), new ItemStack(ZSSItems.bomb, 1, BombType.BOMB_STANDARD.ordinal()));
	private static final MerchantRecipe waterBomb = new MerchantRecipe(new ItemStack(Item.emerald, 12), new ItemStack(ZSSItems.bomb, 1, BombType.BOMB_WATER.ordinal()));
	private static final MerchantRecipe fireBomb = new MerchantRecipe(new ItemStack(Item.emerald, 16), new ItemStack(ZSSItems.bomb, 1, BombType.BOMB_FIRE.ordinal()));
	private static final MerchantRecipe bombSeeds = new MerchantRecipe(new ItemStack(ZSSItems.bombFlowerSeed), new ItemStack(Item.emerald, 4));
	...
	public boolean interact(EntityPlayer player) {
		if (isEntityAlive() && getCustomer() == null && !player.isSneaking()) {
			...
			if (stack != null && trades != null) {
				...
				} else if (stack.getItem() == Item.fishRaw) {
					if (insertBombTrade(waterBomb)) {
						--stack.stackSize;
						chat = "chat.zss.npc.barnes.trade.water";
						openGui = false;
					}
				} else if (stack.getItem() == Item.magmaCream) {
	...
	public boolean addBombBagTrade() {
		return (Config.enableTradeBombBag() && insertBombTrade(new MerchantRecipe(new ItemStack(Item.emerald, Config.getBombBagPrice()), new ItemStack(ZSSItems.bombBag))));
	}
	...
	public static boolean convertFromVillager(EntityVillager villager, EntityPlayer player, ItemStack stack) {
		if (stack != null && stack.getItem() == Item.gunpowder) {
