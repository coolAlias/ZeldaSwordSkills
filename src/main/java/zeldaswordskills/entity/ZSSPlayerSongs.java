...
public class ZSSPlayerSongs
{
	...
	public boolean learnSong(AbstractZeldaSong song, List<SongNote> notes) {
		...
		if (addSong) {
			...
			if (!player.worldObj.isRemote) {
				...
				PacketDispatcher.sendPacketToPlayer(new LearnSongPacket(song, notes).makePacket(), (Player) player);
			}
		}
		return true;
	}
	...
	public boolean removeSong(AbstractZeldaSong song) {
		if (knownSongs.contains(song)) {
			...
			if (player instanceof Player) {
				PacketDispatcher.sendPacketToPlayer(new LearnSongPacket(song, true).makePacket(), (Player) player);
			}
			return true;
		}
		return false;
	...
	public void resetKnownSongs() {
		...
		scarecrowTime = 0;
		if (player instanceof Player) {
			PacketDispatcher.sendPacketToPlayer(new LearnSongPacket(true).makePacket(), (Player) player);
		}
	}
	...
	public void setHorseRidden(EntityHorse horse) {
		if (horse.entityId == horseId) {
			setHorseCoordinates(horse);
		} else if (horse.isTame() && horse.getOwnerName().equals(player.getCommandSenderName())) {
			this.horseId = horse.entityId;
	...
	public void loadNBTData(NBTTagCompound compound) {
		NBTTagList songs = compound.getTagList("KnownSongs");
		knownSongs.clear();
		for (int i = 0; i < songs.tagCount(); ++i) {
			NBTTagCompound tag = (NBTTagCompound) songs.tagAt(i);
		...
		if (compound.hasKey("ScarecrowNotes")) {
			try {
				...
				LogHelper.severe("Exception thrown while loading Scarecrow's Song notes: " + e.getMessage());
			}
		}
		...
		if (compound.hasKey("WarpList")) {
			NBTTagList warpList = compound.getTagList("WarpList");
			for (int i = 0; i < warpList.tagCount(); ++i) {
				NBTTagCompound warpTag = (NBTTagCompound) warpList.tagAt(i);
		...
		if (compound.hasKey("CuredNpcs")) {
			NBTTagList npcs = compound.getTagList("CuredNpcs");
			for (int i = 0; i < npcs.tagCount(); ++i) {
				NBTTagCompound npc = (NBTTagCompound) npcs.tagAt(i);
