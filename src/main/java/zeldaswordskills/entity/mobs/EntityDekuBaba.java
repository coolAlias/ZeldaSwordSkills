...
public class EntityDekuBaba extends EntityDekuBase implements IEntityBombEater, IEntityDynamic, IEntityCustomTarget
{
	...
	public EntityDekuBaba(World world) {
		super(world);
		// TODO would be very cool if hitbox of head moved with it when it performs its 'lunge' attack
	...
	public void entityInit() {
		...
		dataWatcher.addObject(DIFFICULTY_INDEX, worldObj.difficultySetting.getDifficultyId());
	...
	public void performAction(int action_id, IEntityDynamicAI ai) {
		...
		if (isConfused()) {
			...
			if (target instanceof IEntityBombIngestible && canAttack() && TargetUtils.isTargetInFrontOf(this, target, 15F)) {
				IEntityBombIngestible bomb = (IEntityBombIngestible) target;
				if (ZSSEntityInfo.get(this).onBombIngested(bomb)) {
					...
					bomb.setExplosionDamage((float) damage * worldObj.difficultySetting.getDifficultyId());
	...
	protected boolean didAttackCauseProne(DamageSource source, float amount) {
		...
		} else if (isAttack(action.id) && getActionTime(action.id) > worldObj.difficultySetting.getDifficultyId()) {
	...
	public void onUpdate() {
		...
		if (!worldObj.isRemote) {
			...
			for (int i = 0; i < 1; ++i) {
				worldObj.spawnParticle("spell",
						posX - look.xCoord * 0.5D + 0.1D * rand.nextGaussian(),
						this.boundingBox.maxY + 0.25D + 0.15D * rand.nextGaussian(),
		...
		if (getActionTime(ACTION_SPROUT.id) > ACTION_SPROUT.getActionFrame(1.0F)) {
			...
			if (target != null && action != ACTION_PRONE) {
				...
				double dy = target.posY - MathHelper.floor_double(this.boundingBox.minY + 0.5D);
