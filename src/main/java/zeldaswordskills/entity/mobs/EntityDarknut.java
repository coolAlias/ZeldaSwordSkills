...
public class EntityDarknut extends EntityMob implements IEntityBackslice, IEntityEvil, IEntityLootable, IParryModifier, IPowerAttacker, IEntityVariant
{
	...
	protected void applyEntityAttributes() {
		...
		getEntityAttribute(SharedMonsterAttributes.attackDamage).setAttribute(5.0D); // unarmed
		getEntityAttribute(SharedMonsterAttributes.movementSpeed).setAttribute(0.225D);
		getEntityAttribute(SharedMonsterAttributes.knockbackResistance).setAttribute(0.25D);
	...
	public EntityDarknut setType(int type) {
		...
		getEntityAttribute(SharedMonsterAttributes.maxHealth).setAttribute((type > 0 ? 100.0D : 50.0D));
		getEntityAttribute(SharedMonsterAttributes.attackDamage).setAttribute((type > 0 ? 7.0D : 5.0D));
	...
	protected void addRandomArmor() {
		...
		setCurrentItemOrArmor(ArmorIndex.EQUIPPED_CHEST, new ItemStack(Item.plateIron));
	...
	private void applyArmorAttributeModifiers(boolean wearingArmor) {
		AttributeInstance moveAttribute = getEntityAttribute(SharedMonsterAttributes.movementSpeed);
		moveAttribute.removeModifier(armorMoveBonus);
		AttributeInstance knockbackAttribute = getEntityAttribute(SharedMonsterAttributes.knockbackResistance);
	...
	public int getTotalArmorValue() {
		return Math.min(20, super.getTotalArmorValue() + (2 * worldObj.difficultySetting));
	}
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
		...
		if (isEntityInvulnerable() || isSpinning()) {
			...
			amount = damageDarknutArmor(amount * (1.25F - (0.25F * (float) worldObj.difficultySetting)));
			if (amount < 0.5F) {
				return false;
			...
			if (isArmored()) {
				if (TargetUtils.isTargetInFrontOf(this, source.getEntity(), 120)) {
					...
					if (isPlayer && ZSSPlayerSkills.get((EntityPlayer) source.getEntity()).isSkillActive(SkillBase.backSlice)) {
						return super.attackEntityFrom(source, amount);
					} else if (amount > ((float) worldObj.difficultySetting * 2.0F)) {
	...
	public float getOffensiveModifier(EntityLivingBase entity, ItemStack stack) {
		return (0.1F * (float) worldObj.difficultySetting);
	}
	...
	protected void onArmorDestroyed() {
		if (getType() > 0) {
			setCurrentItemOrArmor(ArmorIndex.EQUIPPED_CHEST, new ItemStack(Item.plateChain));
		} else {
	...
	protected DamageSource getDamageSource(int flag) {
		switch(flag) {
		case POWER_ATTACK_FLAG: return new DamageSourceBaseDirect("mob", this).setDamageBypassesArmor();
	...
	public int getChargeTime() {
		return 28 - (worldObj.difficultySetting * 5);
	}
	...
	protected void dropFewItems(boolean recentlyHit, int lootingLevel) {
		...
		switch(n) {
		case 0:	entityDropItem(new ItemStack(Item.flint), 0.0F); break;
		case 1: entityDropItem(new ItemStack(Item.coal), 0.0F); break;
		default: entityDropItem(new ItemStack(Item.ingotIron), 0.0F); break;
		}
	}
	...
	protected void dropRareDrop(int rarity) {
		...
		default: entityDropItem(new ItemStack(Item.painting), 0.0F);
		}
	}

	@Override
	public float getLootableChance(EntityPlayer player, WhipType whip) {
		return (getCurrentItemOrArmor(ArmorIndex.EQUIPPED_CHEST) == null ? 0.25F : 0.0F);
	}
	...
	public EntityLivingData onSpawnWithEgg(EntityLivingData data) {
		...
		addRandomArmor();
		if (rand.nextFloat() < (0.05F * (float) worldObj.difficultySetting)) {
			setType(1);
		}
		return data;
	}
