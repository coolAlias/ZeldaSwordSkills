...
public class ZSSPlayerSkills
{
	...
	private void removeSkill(SkillBase skill) {
		...
		if (player instanceof Player) {
			PacketDispatcher.sendPacketToPlayer(new SyncSkillPacket(dummy).makePacket(), (Player) player);
		}
	}
	...
	public void resetSkills() {
		...
		crestsGiven = 0;
		if (player instanceof Player) {
			PacketDispatcher.sendPacketToPlayer(new SyncPlayerInfoPacket(ZSSPlayerInfo.get(player)).makePacket(), (Player) player);
		}
	}
