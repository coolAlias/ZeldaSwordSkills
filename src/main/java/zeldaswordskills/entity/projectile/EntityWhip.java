...
public class EntityWhip extends EntityThrowable
{
	...
	protected boolean canGrabBlock(Block block, int x, int y, int z, int side) {
		...
		case WHIP_MAGIC:
			...
			if (block instanceof BlockSandStone || block instanceof BlockMushroomCap || 
					(block.blockMaterial.blocksMovement() && block.getBlockHardness(worldObj, x, y, z) > 1.0F)) {
				return true;
	...
	protected boolean isSideClear(int x, int y, int z) {
		Material m = worldObj.getBlockMaterial(x, y, z);
		...
		if (mop.typeOfHit == EnumMovingObjectType.TILE) {
			...
			if (!isInGround() && ticksExisted < getMaxDistance()) {
				...
				if (canGrabBlock(block, mop.blockX, mop.blockY, mop.blockZ, mop.sideHit)) {
					...
					hitY = mop.blockY;
					hitZ = mop.blockZ;
				} else if (canBreakBlock(block, block.blockMaterial, mop.blockX, mop.blockY, mop.blockZ, mop.sideHit)) {
					if (!worldObj.isRemote) {
						...
						worldObj.destroyBlock(mop.blockX, mop.blockY, mop.blockZ, drop);
						setDead();
					}
			...
			if (mop.entityHit instanceof EntityLivingBase) {
				...
				if (inflictDamage && target.getCurrentItemOrArmor(ArmorIndex.EQUIPPED_CHEST) != null) {
	...
	private boolean shouldSwing() {
		...
		if (isDead) { // in case IWhipBlock killed the whip entity
			...
			if (ticksInGround > 10 && !worldObj.isRemote) {
				WorldUtils.activateButton(worldObj, block.blockID, hitX, hitY, hitZ);
				setDead();
			}
			return false;
			...
			if (thrower.worldObj.isRemote) {
				...
				if (swingVec != null) {
					...
					if (thrower.fallDistance > 0 && thrower.motionY < 0) {
						...
						PacketDispatcher.sendPacketToServer(new FallDistancePacket(thrower, -0.467F).makePacket());
					...
					++swingTicks; // increment at end
					PacketDispatcher.sendPacketToServer(new FallDistancePacket(thrower, 0.0F).makePacket());
