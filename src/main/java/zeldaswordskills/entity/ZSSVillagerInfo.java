...
public class ZSSVillagerInfo implements IExtendedEntityProperties
{
		...
		if (villager.isChild()) {
			...
			if (info.incrementSkulltulaTokens()) {
				...
				case 50:
					...
					break;
				case 100:
					reward = new ItemStack(Item.emerald, 64);
			...
			ItemStack reward = new ItemStack(Item.emerald, 64);
	...
	public void addHunterTrade(EntityPlayer player, ItemStack toBuy, int price) {
		...
		if (MerchantRecipeHelper.addToListWithCheck(villager.getRecipes(player), new MerchantRecipe(toBuy, new ItemStack(Item.emerald, price)))) {
