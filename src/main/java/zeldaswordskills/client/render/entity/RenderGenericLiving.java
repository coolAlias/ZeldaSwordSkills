...
public class RenderGenericLiving extends RenderLiving
{
	...
	@Override
	public void doRenderLiving(EntityLiving entity, double dx, double dy, double dz, float yaw, float partialTick) {
		if (entity instanceof IBossDisplayData) {
			BossStatus.setBossStatus((IBossDisplayData) entity, true);
		}
		super.doRenderLiving(entity, dx, dy, dz, yaw, partialTick);
	}
	...
	protected void renderEquippedItems(EntityLivingBase entity, float partialTick) {
		...
		ItemStack helm = entity.getCurrentItemOrArmor(ArmorIndex.EQUIPPED_HELM);
		Item item;
		float f1;
		...
		if (helm != null) {
			...
			if (item instanceof ItemBlock) {
				if (is3D || RenderBlocks.renderItemIn3d(Block.blocksList[item.itemID].getRenderType())) {
					f1 = 0.625F;
			...
			} else if (item == Item.skull) {
				f1 = 1.0625F;
				GL11.glScalef(f1, -f1, -f1);
				String s = "";
				if (helm.hasTagCompound() && helm.getTagCompound().hasKey("SkullOwner")) {
					s = helm.getTagCompound().getString("SkullOwner");
				}
				TileEntitySkullRenderer.skullRenderer.func_82393_a(-0.5F, 0.0F, -0.5F, 1, 180.0F, helm.getItemDamage(), s);
			}
		...
		if (itemstack != null && itemstack.getItem() != null) {
			...
			if (item instanceof ItemBlock && (is3D || RenderBlocks.renderItemIn3d(Block.blocksList[item.itemID].getRenderType()))) {
				f1 = 0.5F;
