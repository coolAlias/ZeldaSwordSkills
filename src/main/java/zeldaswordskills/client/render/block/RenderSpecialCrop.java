 ...
 * 
 * A crop is considered 'mature' if it is an IGrowable and {@code IGrowable#isMature},
 * or if it is not an IGrowable and it has metadata of 7 (ignoring higher bits). 
 * 
...
public class RenderSpecialCrop implements ISimpleBlockRenderingHandler 
{
	...
	public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer) {
		...
		if (block instanceof IGrowable) {
			isMature = ((IGrowable) block).isMature(Minecraft.getMinecraft().theWorld, x, y, z);
		} else {
