...
public class GuiMagicMeterText extends AbstractGuiOverlay
{
	...
	protected void setup(ScaledResolution resolution) {
		...
		this.width = this.mc.fontRenderer.getStringWidth(this.text);
		this.height = this.mc.fontRenderer.FONT_HEIGHT - DEFAULT_PADDING; // font height seems to include some empty space - remove it
		if (Config.isMagicMeterEnabled) {
			...
			this.y = (this.getVerticalAlignment() == VALIGN.BOTTOM ? this.meter.getTop() - this.mc.fontRenderer.FONT_HEIGHT : this.meter.getBottom() + GuiMagicMeter.PADDING);
		} else {
	...
	protected void render(ScaledResolution resolution) {
		this.mc.fontRenderer.drawString(this.text, this.getLeft(), this.getTop(), 0xFFFFFF, true);
	}
}
