...
public abstract class GuiMusicBase extends GuiScreen
{
	...
	public void drawScreen(int mouseX, int mouseY, float f) {
		...
		if (song != null) {
			...
			fontRenderer.drawString(s, guiLeft + (xSize / 2) - (fontRenderer.getStringWidth(s) / 2), guiTop + 3, 0xFFFFFF);
		}
	...
	protected void keyTyped(char c, int key) {
		...
		if (key == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_ATTACK].keyCode) {
			playedNote = PlayableNote.D2; // high D
		} else if (key == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_DOWN].keyCode) {
			playedNote = PlayableNote.F1; // low F
		} else if (key == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_LEFT].keyCode) {
			playedNote = PlayableNote.B2; // high B
		} else if (key == ZSSKeyHandler.keys[ZSSKeyHandler.KEY_RIGHT].keyCode) {
			playedNote = PlayableNote.A2; // high A
		} else if (key == mc.gameSettings.keyBindJump.keyCode) {
	...
	protected void onNotePlayed(SongNote note) {
		...
		mc.thePlayer.playSound(Sounds.OCARINA, 3.0F, f);
