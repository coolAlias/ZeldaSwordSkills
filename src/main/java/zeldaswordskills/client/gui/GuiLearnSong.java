...
public class GuiLearnSong extends GuiMusicBase
{
	...
	public void onGuiClosed() {
		...
		if (song != null) {
			if (ticksSinceLastNote > song.getMinDuration()) {
				PacketDispatcher.sendPacketToServer(new LearnSongPacket(song).makePacket());
			} else {
				PacketDispatcher.sendPacketToServer(new PlayRecordPacket(null, x, y, z).makePacket());
	...
	protected void onNoteAdded() {
		...
		if (melody.get(i) != songToLearn.getNotes().get(i)) {
			...
			PacketDispatcher.sendPacketToServer(new PlayRecordPacket(song.getSoundString(), x, y, z).makePacket());
