...
public class BlockWarpStone extends Block implements ILiftable, ISmashable
{
	...
	public BlockWarpStone(int id) {
		super(id, Material.rock);
		setHardness(50.0F);
		setResistance(2000.0F);
		setStepSound(soundStoneFootstep);
		setTextureName(ModInfo.ID + ":warp_stone");
		...
		if (stack != null && stack.getItem() instanceof ItemInstrument) {
			...
			if (!world.isRemote) {
				...
				if (song != null) {
					...
					LogHelper.warning(String.format("Warp stone at %d/%d/%d had invalid metadata: did not return a song!", x, y, z));
				}
			}
			return true;
		} else {
