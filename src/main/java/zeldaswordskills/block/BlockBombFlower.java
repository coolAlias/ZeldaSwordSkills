...
public class BlockBombFlower extends BlockCrops implements IBoomerangBlock, IExplodable, IGrowable, IWhipBlock
{
	@SideOnly(Side.CLIENT)
	private Icon[] iconArray;

	public BlockBombFlower(int id) {
		super(id);
	...
	protected int getSeedItem() {
		return ZSSItems.bombFlowerSeed.itemID;
	}
	...
	protected boolean canThisPlantGrowOnThisBlockID(int blockId) {
		return blockId > 0 && Block.blocksList[blockId].blockMaterial == Material.rock;
	}

	@Override
	public boolean canBlockStay(World world, int x, int y, int z) {
		int soil = world.getBlockId(x, y - 1, z);
		return canThisPlantGrowOnThisBlockID(soil) && hasLava(world, x, y - 1, z);
	}
	...
	public boolean isMature(World world, int x, int y, int z) {
		return getGrowthStage(world.getBlockMetadata(x, y, z)) == 3;
	}

	@Override
	public Result applyBonemeal(World world, int x, int y, int z) {
		int meta = world.getBlockMetadata(x, y, z);
		if (getGrowthStage(meta) == 3) {
			return Result.DENY;
		} else if (!world.isRemote) { 
			meta = Math.min(7, meta + MathHelper.getRandomIntegerInRange(world.rand, 2, 5));
