...
public class TileEntityDungeonCore extends TileEntityDungeonBlock
{
	...
	public void updateEntity() {
		...
		if (bossBattle == null && isOpened && worldObj.getClosestPlayer(xCoord + 0.5D, yCoord + 0.5D, zCoord + 0.5D, (double)(box.getXSize() - 2) / 2.0D) != null) {
		...
		if (isBossRoom) {
			...
			if (tag.getBoolean("hasBossBattle")) {
				...
				if (bossBattle != null) {
					...
					LogHelper.log(Level.WARNING, "Error retrieving Boss Battle while loading Dungeon Core from NBT");
				}
			}
		}
