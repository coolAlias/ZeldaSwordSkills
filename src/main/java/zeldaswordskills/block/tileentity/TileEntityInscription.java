...
public class TileEntityInscription extends TileEntity
{
	...
	public void setSong(AbstractZeldaSong song) {
		if (song != null && !song.canLearnFromInscription(worldObj, xCoord, yCoord, zCoord, blockType, blockMetadata)) {
			LogHelper.warning(String.format("%s cannot be learned from inscriptions; coordinates: %d/%d/%d", song.getDisplayName(), xCoord, yCoord, zCoord));
			return;
		}
		this.song = song;
	...
	public boolean onActivated(EntityPlayer player) {
		if (song == null) {
			LogHelper.warning(String.format("TileEntityInscription at %d/%d/%d does not have a valid song!", xCoord, yCoord, zCoord));
			return false;
		}
