		...
		if (player != null && ZSSPlayerInfo.get(player) != null) {
			...
			if (skill != null) {
				...
				if (skill instanceof SwordBasic && skill.isLockedOn()) {
					if (skills.hasSkill(SkillBase.leapingBlow) && !skills.isSkillActive(SkillBase.leapingBlow) && isVanillaKeyPressed(mc.gameSettings.keyBindJump) && mc.thePlayer.isBlocking()) {
