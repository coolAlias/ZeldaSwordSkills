...
public class ItemBombFlowerSeed extends ItemSeeds {
	...
	public ItemBombFlowerSeed(int id) {
		super(id, ZSSBlocks.bombFlower.blockID, Block.stone.blockID);
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
		...
		int plantId = getPlantID(world, x, y, z);
		Block plant = Block.blocksList[plantId];
		if (plant != null && plant.canPlaceBlockAt(world, x, y, z)) {
			if (!world.isRemote) {
				world.setBlock(x, y, z, plant.blockID);
			}
			--stack.stackSize;
			return true;
		}
		...
		EntityItem item = new EntityItem(world, entity.posX, entity.posY, entity.posZ, stack) {
			...
			public void onUpdate() {
				...
				if (!worldObj.isRemote && ticksExisted > 80 && worldObj.rand.nextInt(128) == 0) {
					...
					if (flag) {
						worldObj.setBlock(i, j, k, ZSSBlocks.bombFlower.blockID);
