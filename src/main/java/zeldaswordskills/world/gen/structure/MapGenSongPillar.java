...
public class MapGenSongPillar extends ZSSMapGenBase
{
	...
	public void generate(IChunkProvider provider, World world, Random rand, int chunkX, int chunkZ) {
		...
		boolean flag2 = (biome == BiomeGenBase.plains && rand.nextFloat() < 0.35F);
	...
	private void doGenerate(World world, Random rand, int x, int y, int z) {
		...
		for (int i = x - 1; i <= x + 1; ++i) {
			for (int k = z - 1; k <= z + 1; ++k) {
				...
				world.setBlock(i, y, k, Block.stoneBrick.blockID, meta, 2);
			}
		}
		world.setBlock(x, y, z + 2, Block.stoneBrick.blockID, (rand.nextFloat() < 0.4F ? 1 : 0), 2);
		world.setBlock(x, y, z - 2, Block.stoneBrick.blockID, (rand.nextFloat() < 0.4F ? 1 : 0), 2);
		world.setBlock(x + 2, y, z, Block.stoneBrick.blockID, (rand.nextFloat() < 0.4F ? 1 : 0), 2);
		world.setBlock(x - 2, y, z, Block.stoneBrick.blockID, (rand.nextFloat() < 0.4F ? 1 : 0), 2);
		if (song == null) {
			if (world.isBlockSolidOnSide(x + 2, y, z + 2, ForgeDirection.UP) && rand.nextFloat() < 0.35F) {
				StructureGenUtils.setBlockIfReplaceable(world, x + 2, y + 1, z + 2, Block.stoneBrick.blockID, 3);
			}
			if (world.isBlockSolidOnSide(x - 2, y, z + 2, ForgeDirection.UP) && rand.nextFloat() < 0.35F) {
				StructureGenUtils.setBlockIfReplaceable(world, x - 2, y + 1, z + 2, Block.stoneBrick.blockID, 3);
			}
			...
			world.setBlock(x, y + 1, z, Block.stoneBrick.blockID, 2, 2);
			world.setBlock(x, y + 2, z, Block.stoneBrick.blockID, 1, 2);
			if (rand.nextFloat() < 0.5F) {
				world.setBlock(x, y + 3, z, Block.stoneBrick.blockID, 1, 2);
				world.setBlock(x, y + 4, z, Block.stoneBrick.blockID, 1, 2);
				if (rand.nextFloat() < 0.5F) {
					world.setBlock(x + 1, y + 4, z, Block.stairsStoneBrick.blockID, 5, 2);
				}
				if (rand.nextFloat() < 0.5F) {
					world.setBlock(x - 1, y + 4, z, Block.stairsStoneBrick.blockID, 4, 2);
				}
				if (rand.nextFloat() < 0.5F) {
					world.setBlock(x, y + 4, z + 1, Block.stairsStoneBrick.blockID, 7, 2);
				}
				if (rand.nextFloat() < 0.5F) {
					world.setBlock(x, y + 4, z - 1, Block.stairsStoneBrick.blockID, 6, 2);
				}
			}
		} else {
			if (world.isBlockSolidOnSide(x + 2, y, z + 2, ForgeDirection.UP)) {
				StructureGenUtils.setBlockIfReplaceable(world, x + 2, y + 1, z + 2, Block.stoneBrick.blockID, 3);
			}
			if (world.isBlockSolidOnSide(x - 2, y, z + 2, ForgeDirection.UP)) {
				StructureGenUtils.setBlockIfReplaceable(world, x - 2, y + 1, z + 2, Block.stoneBrick.blockID, 3);
			...
			world.setBlock(x, y + 2, z, Block.stoneBrick.blockID, 1, 2);
			world.setBlock(x, y + 3, z, Block.stoneBrick.blockID, 1, 2);
			world.setBlock(x, y + 4, z, ZSSBlocks.secretStone.blockID, 3, 2); // stoneBrick secret stone
			world.setBlock(x + 1, y + 4, z, Block.stairsStoneBrick.blockID, 5, 2);
			world.setBlock(x - 1, y + 4, z, Block.stairsStoneBrick.blockID, 4, 2);
			world.setBlock(x, y + 4, z + 1, Block.stairsStoneBrick.blockID, 7, 2);
			world.setBlock(x, y + 4, z - 1, Block.stairsStoneBrick.blockID, 6, 2);
			world.setBlock(x, y + 5, z, ZSSBlocks.inscription.blockID, 0, 2);
			TileEntity te = world.getBlockTileEntity(x, y + 5, z);
		...
		for (int i = x - 1; i <= x + 1; ++i) {
			for (int j = y - 1; j <= y; ++j) {
				for (int k = z - 1; k <= z + 1; ++k) {
					...
					block2 = (blockId > 0 ? Block.blocksList[blockId] : null);
					blockId = world.getBlockId(i, j + 4, k);
					if (block2 instanceof IDungeonBlock || (blockId > 0 && Block.blocksList[blockId] instanceof IDungeonBlock)) {
						return false;
					} else if (j < y) {
						if (block2 == null || !block2.blockMaterial.isSolid() || block2.blockMaterial == Material.leaves) {
							return false;
						}
					} else if (block2 != null && block2.blockMaterial.isSolid()) {
